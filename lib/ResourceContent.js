...
class ResourceContent {
	 ...
	 */
	constructor({buffer, string, createStream, stream}) {
		if (buffer && createStream || buffer && string || string && createStream || buffer && stream ||
				string && stream || createStream && stream) {
			throw new Error("Cannot create ResourceContent: Please set only one content parameter. " +
	...
	async getBuffer() {
		if (this._contentDrained) {
			throw new Error(`Content stream has been drained. ` +
	 ...
	 */
	_setString(string) {
		this._setBuffer(Buffer.from(string, "utf8"));
	}
	...
	getStream() {
		...
		// If a stream instance is being returned, it will typically get drained by the consumer.
	...
	_getStream() {
		if (this._streamDrained) {
			throw new Error(`Content stream is flagged as drained.`);
		}
