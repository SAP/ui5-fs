...
class FileSystem extends AbstractAdapter {
            ...
            for (let i = matches.length - 1; i >= 0; i--) {
                promises.push(new Promise((resolve, reject) => {
                    ...
                    if (relPath === null) {
                        ...
                        log.verbose(`Failed to resolve virtual path of glob match '${virPath}': Path must start with ` +
        ...
        if (sourceMetadata && sourceMetadata.adapter === ADAPTER_NAME && sourceMetadata.fsPath) {
            ...
            */
            if (sourceMetadata.fsPath !== fsPath && !sourceMetadata.contentModified) {
                ...
                log.silly(`Resource hasn't been modified, target path equals source path. Skipping write to ${fsPath}`);
                if (readOnly) {
                    await chmod(fsPath, READ_ONLY_MODE);
                }
                return;
    ...
    _resolveToFileSystem(relPath) {
        ...
        if (!fsPath.startsWith(this._fsBasePath)) {
            ...
            throw new Error(`Error while resolving internal virtual path: '${relPath}' resolves ` +
