...
class FileSystem extends AbstractAdapter {
	...
	async _write(resource, {drain, readOnly}) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!resource._modified && resourceSource.adapter === "FileSystem" && resourceSource.fsPath) {
				if (resourceSource.fsPath === fsPath) {
					// Resource is not modified and src/dest are equal => Write can be skipped
					skippedResources++;
					return;
				}
				copiedResources++;
...
setInterval(() => {
	...
	console.log(`Skipped ${skippedResources}/${totalResources} resources...`);
}, 1000);
