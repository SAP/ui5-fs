...
class FileSystem extends AbstractAdapter {
	...
	async _runGlob(patterns, options = {nodir: true}, trace) {
		...
		const globbyPatternsKey = globbyPatterns.join(":");
		if (globbyPatterns.length > 0) {
			let matches = this._globCache.get(globbyPatternsKey);
			if (!matches) {
				console.log(`Globby: ${this._fsBasePath} - ${globbyPatternsKey}`);
				matches = await globby(globbyPatterns, opt);
				this._globCache.set(globbyPatternsKey, matches);
			} else {
				console.log(`Matches from cache: ${this._fsBasePath} - ${globbyPatternsKey}`);
			}

			for (let i = matches.length - 1; i >= 0; i--) {
				promises.push(new Promise((resolve, reject) => {
					const match = matches[i];
					const fsPath = path.join(this._fsBasePath, match);
					const virPath = (this._virBasePath + match);

					const fileCacheEntry = this._fileCache.get(virPath);
					if (fileCacheEntry) {
						resolve(new Resource(fileCacheEntry));
						return;
					}
					...
					fs.stat(fsPath, (err, stat) => {
						if (err) {
							...
							this._fileCache.set(virPath, options);
	...
	_byPath(virPath, options, trace) {
		...
		const fileCacheEntry = this._fileCache.get(virPath);
		if (fileCacheEntry) {
			return Promise.resolve(new Resource(fileCacheEntry));
		}
