...
class AbstractAdapter extends AbstractReaderWriter {
    ...
    constructor({ virBasePath, excludes = [], project }) {
        ...
        if (!virBasePath.endsWith("/")) {
            throw new Error(`Unable to create adapter: Virtual base path must end with a slash but is '${virBasePath}'`);
        }
        ...
        if (this._project) {
            ...
            if (resource.hasProject()) {
                if (resource.getProject() !== this._project) {
                    throw new Error(`Unable to write resource associated with project ` +
    ...
    _resolveVirtualPathToBase(inputVirPath, writeMode = false) {
        ...
        if (!writeMode) {
            ...
            throw new Error(`Failed to write resource with virtual path '${inputVirPath}': Path must start with ` +
