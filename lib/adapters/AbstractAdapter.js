...
class AbstractAdapter extends AbstractReaderWriter {
	...
	constructor({virBasePath, excludes = [], project}) {
		...
		this._excludes = excludes.map((pattern) => `!${pattern}`);
	...
	isExcludedPath(virPath) {
		const aMatches = this._excludes.filter((pattern) => {
			// Due tue negation in the consturctor we need to flip the negation
			return minimatch(virPath, pattern, {flipNegate: true});
		});
