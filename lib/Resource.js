...
class Resource {
	...
	*/
	 ...
	 */
	setBuffer(buffer, encoding = null) {
	 ...
	 */
	setString(string) {
		this.setBuffer(Buffer.from(string, "utf8"), "utf8");
	}
	...
	getStream() {
		...
		if (this._buffer) {
			...
			if (this._encoding) {
				bufferStream.setEncoding(this._encoding);
				// console.log(`Encoding set to ${this._encoding} for stream of ${this.getPath()}`);
			}
	...
	_getBufferFromStream() {
		...
		return this._buffering = new Promise((resolve, reject) => {
			...
			contentStream.on("end", () => {
				...
				this.setBuffer(buffer, this._encoding);
