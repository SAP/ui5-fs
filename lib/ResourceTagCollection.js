...
class ResourceTagCollection {
    ...
    _validateTag(tag) {
        ...
        if (!tagNamespaceRegExp.test(tagNamespace)) {
            throw new Error(`Invalid Tag "${tag}": Namespace part must be alphanumeric, lowercase and start with a letter`);
        }
        ...
        if (!this.acceptsTag(tag)) {
            throw new Error(`Tag "${tag}" not accepted by this collection. Accepted tags are: ` +
    ...
    _validateValue(value) {
        ...
        if (!["string", "number", "boolean"].includes(type)) {
            throw new Error(`Invalid Tag Value: Must be of type string, number or boolean but is ${type}`);
        }
    }
}
export default ResourceTagCollection;
//# sourceMappingURL=ResourceTagCollection.js.map