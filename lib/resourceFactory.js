...
const ZipAdapter = require("./adapters/ZipArchive");
...
const resourceFactory = {
	...
	*/
	...
	createAdapter({fsBasePath, virBasePath, project, excludes}) {
		const dotZipIdx = fsBasePath.indexOf(".zip/");
		if (dotZipIdx !== -1) {
			// console.log(virBasePath);
			// console.log(fsBasePath);
			// console.log(fsBasePath.substring(0, dotZipIdx + 4));
			// console.log(fsBasePath.substring(dotZipIdx + 4));
			// console.log("");
	 ...
	 */
	createReader({fsBasePath, virBasePath, project, name}) {
		return new ReaderCollection({
			name,
			readers: [resourceFactory.createAdapter({fsBasePath, virBasePath, project})]
		});
	},
