...
class WriterCollection extends AbstractReaderWriter {
    ...
    constructor({ name, writerMapping }) {
        ...
        this._basePathRegex = basePaths.sort().reduce((regex, basePath, idx) => {
            ...
            if (!basePath.startsWith("/")) {
                throw new Error(`Missing leading slash in path mapping '${basePath}' of WriterCollection ${this._name}`);
            }
            if (!basePath.endsWith("/")) {
                throw new Error(`Missing trailing slash in path mapping '${basePath}' of WriterCollection ${this._name}`);
            }
    ...
    _write(resource, options) {
        ...
        if (!basePathMatch || basePathMatch.length < 2) {
            throw new Error(`Failed to find a writer for resource with path ${resourcePath} in WriterCollection ${this._name}. ` +
...
//# sourceMappingURL=WriterCollection.js.map