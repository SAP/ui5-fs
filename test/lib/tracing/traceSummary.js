...
test.serial("traceSummary", async (t) => {
	...
	t.is(myLoggerInstance.verbose.callCount, 1, "Logger has been called exactly once");
...
test.serial("traceSummary no verbose logging", async (t) => {
	...
	t.is(myLoggerInstance.verbose.callCount, 0, "Logger has not been called (due to disabled verbose logging)");
});
